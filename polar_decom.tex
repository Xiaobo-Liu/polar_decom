...
\begin{align*} 
  ...
  \Leftrightarrow  \quad
  Q^*=&\frac{2}{\pi}\int_{0}^{\infty}(Q\Sigma^{-1})^{-1}(t^2I+Q\Sigma^*\Sigma Q^*)^{-1}\mathrm{d}t	\\
    \Leftrightarrow  \quad
  Q^*=&\frac{2}{\pi}\int_{0}^{\infty}(t^2Q\Sigma^{-1}+Q\Sigma )^{-1}\mathrm{d}t	\\
  \Leftrightarrow  \quad
  Q^*=&\frac{2}{\pi}\int_{0}^{\infty}(t^2\Sigma^{-1}+\Sigma )^{-1}\mathrm{d}t\cdot Q^{-1}.	
\end{align*}
Postmultiplying by $Q$ gives
$$
I=\frac{2}{\pi}\int_{0}^{\infty}(t^2\Sigma^{-1}+\Sigma )^{-1}\mathrm{d}t,
$$
...
i=1:n.
\end{equation}
Without loss of generality, we only prove \eqref{svd_int} with $\sigma_i$. Let $t=\sigma_i x$, it follows the right hand side of \eqref{svd_int} becomes
$$
